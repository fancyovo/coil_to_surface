...
- 2026-08-11: `codex/direct-alpha-fast-score` now has the first non-production local-gradient calibration harness. It decodes shared RK4-128 endpoint banks once, shards formal ABI-10 score and four aggressive fixed-level proxies across four GPUs, and compares directional cosine/sign/rank against the formal score at Adam steps 50/120/200. Proxy variants separate moving-axis versus fixed-axis behavior, QR versus normal equations, and 16k versus 8k volume sketches. Production defaults and ABI are unchanged. The gate remains gradient direction rather than proxy score value; no numerical result is accepted yet. See `reports/direct_alpha_fast_score_feasibility_20260811.md`.
