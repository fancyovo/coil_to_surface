...
QH 优化采用一个 3033 万参数的非因果 Transformer Flow Matching 模型作为可逆坐标变换。每条实验轨迹从 32 个随机潜变量中选取最高有效起点，再运行 200 步有限差分 Adam；每步使用 64 个正交随机方向和 128 个对称端点。309 条轨迹的独立复评中位数由起点的 75.996 提高到 90.214，体 QH 误差中位数降低 10.8 倍。32 个真实筛选起点上的配对实验进一步表明，100 步中 Flow 潜空间与标准化原参数空间的中位增益分别为 7.676 和 8.664；因此当前证据支持 Flow 是可用的学习型坐标变换，但不支持它带来普遍优化优势。对 768 个优化位形和 1536 个标准磁面的标定显示：体 QH 与面 QH 的 Spearman 相关系数为 0.944--0.967，GPU $\iota$ 与标准面 $\iota$ 的 Spearman/Pearson 相关系数为 0.991/0.996。结果表明，这条固定预算体诊断路线能够作为 QH 线圈筛选与优化的高吞吐前端；标准磁面求解和 DESC 继续承担少量候选的最终物理验收。
...
对每个候选值 $s_j$，程序先在 $\phi=0$ 截面沿 128 个极角方向求解等值线点，再把这些点各自追踪一个场周期，每周期固定使用 400 步。追踪终点通常不会精确回到同一条拟合等值线。第 $j$ 层第 $k$ 个终点的相对法向漂移定义为
...
其中 $\bar r_j$ 是该层 128 个起点的平均物理半径。分子是终点处的标签偏差；在线性近似下，用它除以 $|\nabla s|$ 就得到终点离目标等值线的法向距离，再除以 $\bar r_j$ 得到与磁面尺寸无关的相对漂移。因此，$d_{jk}=0.05$ 可理解为磁力线走过一个场周期后，法向偏离约为该层平均半径的 5%。

同一层的 128 个 $d_{jk}$ 不能简单取平均，因为少量严重漂移的角区同样会破坏闭合磁面。程序用温度 $0.005$ 的平滑最大值把它们合成风险 $r_j$；该统计接近最大值，但对单个采样点的小扰动连续。随后使用

$$
p_j^{\mathrm{drift}}=\frac{1}{1+\exp[(r_j-0.05)/0.0125]}
$$

把风险变成漂移置信度：相对漂移明显小于 5% 时 $p_j^{\mathrm{drift}}$ 接近 1，明显大于 5% 时接近 0。若该层的射线根已经逼近体拟合的采样半径上限，程序还会用第二个平滑因子降低置信度，避免把 $s$ 外推到没有数据支持的区域；两者乘积记为 $p_j$。物理上外层不应比内层更可信，因此再用加权单调回归把 $p_j$ 修正为随 $s_j$ 不增的曲线。最终有效层定义为这条置信度曲线下面积，

$$
s_{\mathrm{eff}}=\int_0^{s_{\max}}p(s)\,ds,
$$

数值上在上述候选层之间作梯形积分。这个量不是宣称 $s=s_{\mathrm{eff}}$ 必然是一张精确磁面，而是对“从轴向外有多大范围得到追踪支持”的连续概括；它避免了直接选择某个离散最大层时目标函数发生跳变。
...
四次多项式固定零截距。一个边界只有同时满足三项条件才被接受：积分边界残差不超过 $2\times10^{-6}$，边界磁通在 8 个环向截面间的相对标准差不超过 2%，并且 $\psi'(s)$ 在区间内不变号。如果 $s_{\mathrm{eff}}$ 未通过，程序先向内寻找一个通过验收的候选层，再在“通过的内层”和“未通过的外层”之间最多二分 6 次。每次二分都重新做磁通积分和上述三项检查；因此二分是在已知区间内确定可用边界，不是无界的非线性求解。

![图 2(a)：代表性 QH 样本的实际磁通标定。红点是各径向层的截面平均积分，蓝线是固定零截距的多项式；右图给出其导数。](../assets/qh_flow_candidate275_alpha_nu_guarded_29109/alpha/flux_calibration.png)

图 2(a) 中拟合曲线紧随直接积分值，且 $d\psi/ds$ 在整个区间保持同号。这正是磁通标定需要保留的两项性质：用低阶函数平滑离散积分误差，同时不改变由轴到边界的磁通次序。
...
程序在近似等物理体积的 100000 个点上计算场量，并从 10 个径向层中分层选择 30000 点建立联合线性系统。有限 $s$ 拟合误差产生的法向磁场分量先从直场线方程中投影出去，随后同时求解 $d_{lmn\sigma}$ 与四个 $\iota_p$。默认求解器为 FP32 QR，正则系数为 $10^{-7}$。`alpha_relative_l2` 是这 30000 条物理拟合方程去掉正则行后的相对残差，衡量联合解对所建线性系统的自洽程度；当前原生结果没有单独的 $\alpha$ 留出集字段。其余返回量包括投影前法向场相对量 `alpha_normal_B_relative_l2`、$\iota$ 的最小值和最大值，以及线性系统列数。

这一步产生整个受支持体积内的近直场线坐标。完整评估支线继续拟合环向修正 $\nu$，把体坐标转换成 Simsopt 磁面求解所需的近 Boozer 初值。

![图 2(b)：同一代表性 QH 样本上的实际磁力线数据。左图使用几何极角，磁力线明显弯曲；右图使用拟合后的直场线角，三组径向位置上的磁力线接近平行直线。](../assets/qh_flow_candidate275_alpha_nu_guarded_29109/alpha/fieldline_straightening.png)

图 2(b) 不是概念示意，而是同一个实际线圈样本在坐标修正前后的追踪结果。右图仍有有限拟合误差，但斜率随径向位置系统变化，正对应 $\iota(\rho)$ 而非常数旋转变换。
...
除 QS 外，评估器还直接计算线圈长度均值、曲率 P95、最大曲率、线圈间最小距离、线圈到设备旋转轴的最小柱半径、高阶 Fourier 模能量比例和最大电流绝对值。其中高阶能量统计 $m\ge9$ 的几何系数；设备旋转轴是圆柱坐标的 $z$ 轴，不是前文求得的磁轴。这些工程量不参与 $s$、$\psi$、$\alpha$ 或 QS 的物理拟合，只在构造最终优化目标时用于抑制过长、过弯、过近或高频振荡的线圈。
...
训练集含 170755 个 QUASR QH 线圈数据样本，覆盖 $N_{\mathrm{FP}}=2,\ldots,8$。输入采用逐坐标标准化以匹配高斯初始尺度；训练损失随后按 Parseval 等式恢复 Fourier 系数对应的曲线积分距离权重，使低阶几何模保持主导作用。本文使用指数滑动平均（exponential moving average，EMA）第 30000 步的检查点，优化时采用 FP32 RK4-128。Flow 在本方法中的作用是可逆坐标变换：它把相关的线圈形变组织成潜空间方向，从而改变搜索景观。
...
优化效果使用正式 `native_score`、`status=ok` 比例、体 QH 误差、分数组成和独立重评分一致性。分布比较同时报告中位数、高分阈值比例和上尾生存曲线。下文用 P50 表示中位数，用 P95 表示第 95 百分位数。

物理可信度按量选择参考：$s$ 使用独立体点方向误差；$\iota$ 使用标准磁面值；体 QH 使用标准面上 $|B|$ 的非目标 Fourier 模相对方差；近 Boozer 初值使用标准磁面方程残差。Spearman 秩相关系数衡量排序一致性，Pearson 相关系数衡量近似线性关系；误差分布报告 P50/P95。
...
Flow 的作用通过两类证据检验。第一类是在已知 QH 样本附近，分别沿潜空间方向、该方向经 Flow 映射传输后的原参数局部切向，以及独立原参数随机方向扫描正式分数。设 Flow 解码映射为 $F$，中心潜变量为 $\boldsymbol z_0$，单位方向为 $\boldsymbol u$。传输切向用中心差分近似：
...
它表示潜空间方向在中心附近对应的线圈参数变化率。扫描关注达到固定分数下降的物理扰动宽度、有效状态比例和离散曲线粗糙度。

同版本重跑选择 3 个 QUASR QH 样本，覆盖 $(N_{\mathrm{FP}},n_c)=(3,4),(2,5),(4,3)$，中心分数分别为 88.50、52.01 和 89.15。每个中心取 4 个潜空间方向；对潜空间曲线、传输切向直线，以及在标准化原参数空间中具有相同均方根（root mean square，RMS）尺度的独立随机方向，各扫描 31 个双侧位置。去重后共执行 1095 次独立评分。反演再正演使用 FP32 RK4-256，三个中心的线圈位置 RMS 闭合误差为 $2.26\times10^{-8}$--$4.57\times10^{-8}\,\mathrm m$。

![图 7：三个 QUASR QH 中心附近的正式分数景观。实线为潜空间方向，虚线为同一方向传输到原参数空间后的局部切向，点线为等 RMS 的独立原参数随机方向。图中显示 $|\alpha|\le0.033$ 的近邻区间。](assets/landscape_current_20260824/landscape_score_vs_alpha_zoom.png)

图 7 中潜空间曲线与对应切向曲线在中心附近基本重合，随机原参数方向更早出现陡降和无效状态。为减少坐标尺度对结论的影响，下表同时给出方向参数宽度和线圈位置 RMS 位移宽度。宽度定义为从中心向正、负两侧首次下降给定分值的位置之和；表中数值均为“潜空间宽度/对照宽度”在 12 个方向上的中位数。粗糙度比使用非均匀网格二阶差分 RMS，数值小于 1 表示潜空间曲线更平滑。

| 对照方向 | 下降 5 分参数宽度比 | 下降 10 分参数宽度比 | 下降 5 分物理半径比 | 下降 10 分物理半径比 | 二阶粗糙度比 |
|---|---:|---:|---:|---:|---:|
| Flow 传输切向 | 0.985 | 0.994 | 1.047 | 1.153 | 0.984 |
| 独立原参数随机方向 | 6.786 | 6.047 | 3.281 | 3.010 | 0.0605 |

下降 10 分时，潜空间与传输切向分别有 6/12 和 7/12 条曲线在扫描边界仍未达到阈值，因此两者约为 1 的比值只支持“宽度相当”；扫描范围还不足以判断细小差别。随机原参数方向的 12 条曲线全部达到阈值，且潜空间在 12/12 个匹配方向上都具有更大的物理下降半径。二阶粗糙度的中位比为 0.0605，12 个方向中有 11 个更平滑。该结果把 Flow 的作用定位为：它把潜空间方向传输到训练数据支持的相关线圈形变上；沿同一局部切向改用非线性 Flow 路径只产生次要变化。完整范围曲线与等物理位移散点分别见[全范围扫描](assets/landscape_current_20260824/landscape_score_vs_alpha.png)和[物理位移对照](assets/landscape_current_20260824/landscape_score_vs_displacement.png)。

第二类是配对优化。为避免“已经接近局部最优的轨迹中段”把增益人为压到零，实验只使用优化开始前的筛选结果：从 309 条轨迹中平衡抽取 32 条不同轨迹，每个样本都是 32 个随机 Flow 候选中正式分数最高的一个。样本覆盖 $N_{\mathrm{FP}}=2,\ldots,8$、1--5 根基本线圈和 27 种条件组合；起始分数 P10/P50/P90 为 27.54/75.09/80.20。

Flow 分支从筛选时保存的潜变量出发，原参数分支从同一个解码线圈的逐坐标标准化 Fourier token 出发。两侧初始正式分数的最大差为 0.0191。两侧均运行 100 步、每步两个正交方向、中心差分、$\eta=0.01$ 和 $(\beta_1,\beta_2)=(0.7,0.999)$；有限差分半径在潜空间为 0.005，在标准化原参数空间为 0.0025。两个坐标的单位不同，因此这里比较的是各自稳定工作配置下的实际优化效果，而不是声称两侧使用了同一个物理步长。

![图 8：32 个真实筛选起点在 Flow 潜空间和标准化原参数空间中的配对优化。左上为中位进度及 IQR、P10--P90 带；右上为 100 步增益的经验分布；左下逐样本连接两侧结果；右下给出优势与起始质量的关系。](assets/flow_pairs_true_starts_20260824/flow_pairs/analysis_review2/flow_vs_data_optimization.png)

| 成对指标 | Flow 潜空间 | 标准化原参数空间 |
|---|---:|---:|
| 100 步增益 P10/P50/P90 | 4.579 / 7.676 / 12.950 | 5.254 / 8.664 / 21.839 |
| 100 步无增益样本 | 0/32 | 1/32 |
| 共同 137.1 秒预算内增益 P50 | 5.203 | 8.317 |
| 100 步墙钟 P50 | 312.5 s | 217.7 s |
| 最终胜出的匹配样本数 | 10/32 | 22/32 |
| 起始分数 $\ge70$ 的 23 组：增益 P50 | 7.366 | 7.687 |

Flow 最优分减去原参数最优分的 P10/P50/P90 为 $-9.137/-1.031/2.688$，中位数的样本自助法 95% 区间为 $[-1.807,-0.223]$。双侧符号检验和 Wilcoxon 配对检验的 $p$ 值分别为 0.0501 和 0.0475；这是一组探索性实验，数值只用于描述证据强度。原参数空间在低分起点上的少数大增益拉宽了上尾，但把范围限制到起始分数不低于 70 的 23 组后，它仍以 16/23 的胜出数和略高的中位增益占优。Flow 每步还需要 RK4-128 解码，固定 100 步的墙钟中位数是原参数空间的 1.51 倍。

因此，图 7 与图 8 回答的是不同问题。图 7 说明 Flow 学到的相关线圈形变比独立原参数随机方向更宽、更平滑；图 8 则说明这种几何性质没有自动转化为两方向有限差分 Adam 的更高增益。有限差分方向与上升方向的对齐、坐标尺度以及额外解码成本都会影响实际结果。已有的单起点 200 步对照中，Flow 曾达到 93.176，而原参数空间达到 91.783；新实验表明它是确实存在的优势个例，但不能外推为普遍规律。本文因此把 Flow 定位为可用但非必要的学习型重参数化，优化能力的主要证据归于固定预算评分器和有限差分 Adam 的组合。
...
| $\alpha$ | 30000 条物理拟合方程相对 $L^2$ | 766 位形 | P50/P95：0.099/0.602 |
...
![图 9：体微分 QH 与标准磁面 QH 的标定。](../assets/qh_trajectory_face_qs_calibration_20260819/face_vs_volume_qs.png)

图 9 左图在双对数坐标中比较全体积微分 QH 与两类标准面 QH。点云跨越约五个数量级并保持清晰单调关系，说明体指标具备跨优化阶段的排序分辨率。固定探针面整体略低于自适应边界面，反映径向位置差异。右图把面 QH 对逆纵横比作图，并以 Adam 步数着色；在相近尺寸下，后期样本系统性向更低面 QH 移动，表明相关性主要来自 QS 改善，而非磁面缩小。

![图 10：GPU 联合拟合 $\iota$ 与标准磁面 $\iota$ 的比较。](../assets/qh_trajectory_face_qs_calibration_20260819/iota_comparison.png)

图 10 左图显示 GPU 与 Simsopt 的 $\iota$ 大多数沿单位斜线分布，少量离群点集中在标准面求解较弱或拟合尾部较大的样本。中图给出绝对误差分布，右图给出相对误差的十进对数分布；主体集中在 $10^{-3}$ 附近。该结果支持在受支持体积内把三次 $\iota(u)$ 用作定量优化元数据和标准面初值。
...
| 线圈长度、曲率、间距、高阶能量和电流 | 由当前线圈正式计算 | 由当前线圈正式计算 | 不逐项重算；工程分量使用中心附近的一阶近似 | 代理只判断近邻更新方向，接受后的中心会重新计算全部工程量 |
...
![图 11：正式模式延迟、近邻批量均摊时间及代理分数与严格续接分数的关系。](assets/evaluator_modes_current_20260824/analysis/evaluator_modes_overview.png)
...
![图 12：两个正式模式的互斥阶段平均耗时分解。](assets/evaluator_modes_current_20260824/analysis/formal_stage_timing.png)

图 12 使用互斥顶层阶段，避免把 `axis_search` 与其内部网格追踪、候选精修重复相加。独立评估平均 2.797 秒，其中磁轴定位与追踪占 87.7%；严格续接平均 0.847 秒，磁轴阶段降至 0.501 秒，占 59.2%。严格续接中的 $s$ 拟合与独立检验、连续边界与磁通、$\alpha/\iota$ 联合拟合分别占 12.8%、18.0% 和 6.61%。体 QS 归约与分数组合只占约 0.09%，当前正式评估的主要成本来自确定并验证几何坐标，而非最终 QS 公式。
...
![图 13：代表性线圈组与样本自适应选择的较大磁面。](../assets/qh_min_face_qh_full_evaluation_20260819/full/assets/coils_surface.png)

![图 14：标准磁面上的白底彩色 $|B|$ 等高线。颜色表示磁场强度。](../assets/qh_min_face_qh_full_evaluation_20260819/full/assets/boozer_b.png)

图 14 中等高线沿 QH 螺旋方向近似平直，QA 与 QP 误差比 QH 高约三个数量级。Poincare 截面和稠密残差同时确认该边界对应嵌套磁面。DESC 初态和终态保持嵌套，$\iota(\rho)$ 从约 1.444 平滑增至 1.456；50 步内归一化力残差降低约三个数量级，求解在步数上限处停止。完整的 8 张 DESC 图、交互式三维 HTML 和机器可读结果见[代表性样本完整评估报告](../qh_min_face_qh_full_evaluation_20260819.md)。
...
309 条轨迹显示，64 方向有限差分 Adam 能把从 Flow 分布筛出的中等质量起点稳定推入 90--93 分区间，并把体 QH 误差中位数降低约一个数量级。32 个真实起点的配对对照同时表明，当前协议下直接优化标准化原参数并不弱于 Flow 潜空间；因此不能把 309 条轨迹的改善单独归因于 Flow。768 位形和 1536 标准面的标定进一步表明，体 QH 保持对面 QH 的强排序关系，GPU $\iota$ 与标准面值高度一致，$\alpha+\nu$ 能提供低残差标准磁面初值。代表性候选在较大磁面上呈现清晰 QH $|B|$ 结构，并通过 Simsopt、Poincare 与 DESC 的分层验收。
...
| 32 组真实起点配对动态库 | SHA-256 `7d3784e02172f5c3130bcf085efa8120dc059d5007daa4b18680edd97b077f94` |
| 32 组配对协议 | 每条不同轨迹的原始 32 选 1 起点；100 步；2 个正交方向；潜空间/原参数半径 0.005/0.0025；$\eta=0.01$；$\beta_1=0.7$；$\beta_2=0.999$；Flow 使用 FP32 RK4-128 |
...
- [当期 32 组真实起点配对优化汇总](assets/flow_pairs_true_starts_20260824/flow_pairs/analysis_review2/summary.json)
