...
- 2026-08-11 correction: `codex/direct-alpha-fast-score` jobs `36448/36458` tested reduced-density endpoint QR and fixed-geometry negative controls, not the requested full-density warm-start iterative $s$ solve. Their failed total-score gradients therefore do not reject that route (volume-QS cosine remained `0.908/0.886`). Next comparison must use identical endpoints and fixed `h` for exact endpoint QR versus full-grid48 LSQR/LSMR/CGLS initialized from the center coefficients; continue/refine the axis and refit $s$, fixing only the center-selected scalar $s_0$, so the endpoint surface remains $s_q(x)=s_0$. Do not reuse fixed-axis/fixed-$s$ variants. ABI/default score remain unchanged; corrected interpretation and proposed equations are appended to `reports/direct_alpha_fast_score_feasibility_20260811.md`.
