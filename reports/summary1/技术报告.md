...
$y_j(t)$ 和 $z_j(t)$ 采用同一约定。场周期旋转与恒星器对称生成完整线圈组。磁场计算把每根线圈离散为 256 段，CUDA 内核按 Biot--Savart 定律计算 $\boldsymbol B$ 与空间梯度 $\nabla\boldsymbol B$。工程量在每根基本线圈的 160 个等参数点上统计长度和曲率，并对完整对称线圈组计算线圈间最小距离。字段 `coil_min_axis_distance` 保留了历史名称，其当前定义是线圈点到设备旋转轴（$z$ 轴）的最小柱半径。高模能量包含 Fourier 模 $m\ge9$。
...
除 QS 外，评估器直接返回线圈长度均值、曲率 P95、最大曲率、线圈间最小距离、线圈点到设备旋转轴的最小柱半径、高模能量比例和最大电流绝对值。这些量与物理诊断一起构成默认分数的输入。接口字段 `coil_min_axis_distance` 沿用历史名称，度量对象始终是设备旋转轴柱半径，与磁轴无关。
...
优化有效性实验使用 309 条独立轨迹。每条包含 32 个随机起点和 200 个 Adam 更新，共计 9888 个随机 Flow 样本、62109 个正式中心、3955200 个正交方向和 7910400 个近邻端点。对照组从 QUASR QH 测试集无放回抽取 1024 个样本，并在同一原生 C 接口第 10 版（ABI 10）定义下独立评分。
...
同版本重跑选择 3 个 QUASR QH 样本，覆盖 $(N_{\mathrm{FP}},n_c)=(3,4),(2,5),(4,3)$，中心分数分别为 88.50、52.01 和 89.15。每个中心取 4 个潜空间方向；对潜空间曲线、传输切向直线，以及在标准化原参数空间中具有相同均方根（root mean square，RMS）尺度的独立随机方向，各扫描 31 个双侧位置。去重后共执行 1095 次当前 ABI 10 独立评分。反演再正演使用 FP32 RK4-256，三个中心的线圈位置 RMS 闭合误差为 $2.26\times10^{-8}$--$4.57\times10^{-8}\,\mathrm m$。
...
| `coil_min_axis_distance` | 线圈点到设备旋转轴（$z$ 轴）的最小柱半径，单位 m；字段名为历史接口名称 |
