...
### 3.1 增长曲线与 50 step 是否饱和

下图用实线画每个种子的 running best，用同色浅线画实际 current score。右侧柱长是最后 10 步带来的 running-best 增量，黑色短线是其中最后 5 步的增量。

![8 个种子的 score 增长曲线和末段增益](assets/qh_screened_start_adam_20260802/screened_start_adam_score_curves.png)

| 种子后缀 | 最后 10 步增益 | 最后 5 步增益 | 最后 10 轮有效更新 | 判断 |
|---:|---:|---:|---:|---|
| 00 | 1.038 | 0.387 | 10 | 仍在上升，但速度已放缓 |
| 01 | 3.964 | 1.592 | 10 | 明显未饱和 |
| 02 | 0.679 | 0.312 | 10 | 接近缓慢区，但仍未完全饱和 |
| 03 | 0.000 | 0.000 | 0 | 不是正常收敛；被无效端点门控卡住 |
| 04 | 0.607 | 0.383 | 10 | 接近缓慢区，但仍未完全饱和 |
| 05 | 1.527 | 0.837 | 10 | 尚未饱和，且距离 50 仅 `0.4573` |
| 06 | 4.140 | 2.207 | 10 | 上升最快，明显未饱和 |
| 07 | 3.609 | 1.504 | 10 | 明显未饱和 |

因此，50 step 对这一批实验主要是人为截断，而不是自然收敛点。除种子 `03` 外，其余 7 组的 running best 都在第 50 步取得全程最高值；其中 `01/06/07` 在最后 10 步仍有 3.6 至 4.1 分增益。`02/04` 的末段斜率已经较小，可以认为正在接近平台，但现有 50 步证据还不能称其完全饱和。`03` 的平台来自最后 10 轮全部跳过，并不能通过单纯延长相同配置解决。
...
如果下一步仍以突破 50 为目标，比立即换 `(nfp,n_coils)` 更直接的对照是保留相同 128 候选协议，把 50 步延长到 100 或 150 步，并继续保留 running best。理由是除门控卡住的 `03` 外，其余 7 组都在第 50 步取得全程最高 running best，最高的 `49.5427` 也仍在上升，说明 50 步对大多数盆地是人为截断；但 `03` 的大量无效端点说明单纯延长并不能修复所有失败盆地。
