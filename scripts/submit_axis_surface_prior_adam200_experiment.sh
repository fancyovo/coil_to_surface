...
p107=(--account=competition --partition=P107-RTX5090 --qos=qos_p107-rtx5090 --cpus-per-task=4 --mem=32G --array=0-3 --job-name=axis-v2-adam-p107 --export=ALL,WORKER_OFFSET=0)
students=(--account=stu --partition=Students --qos=qos_stu_medium_2gpu --cpus-per-task=12 --mem=48G --array=0-1 --job-name=axis-v2-adam-stu --export=ALL,WORKER_OFFSET=4)
smoke=(--account=competition --partition=P107-RTX5090 --qos=qos_p107-rtx5090 --job-name=axis-v2-adam-smoke --export=ALL)

sbatch --test-only --export=ALL scripts/slurm_prepare_axis_surface_prior_adam200.sh
sbatch --test-only "${smoke[@]}" scripts/slurm_smoke_axis_surface_prior_adam200.sh
sbatch --test-only "${p107[@]}" scripts/slurm_axis_surface_prior_adam200_worker.sh
sbatch --test-only "${students[@]}" scripts/slurm_axis_surface_prior_adam200_worker.sh
sbatch --test-only --export=ALL scripts/slurm_analyze_axis_surface_prior_adam200.sh

mkdir -p "$RUN_ROOT"
cp evaluation/axis_surface_contour_prior_balanced_adam200_abi11_v1.json "$RUN_ROOT/protocol.json"
prepare_job=$(sbatch --parsable --export=ALL scripts/slurm_prepare_axis_surface_prior_adam200.sh)
smoke_job=$(sbatch --parsable --dependency="afterok:$prepare_job" "${smoke[@]}" scripts/slurm_smoke_axis_surface_prior_adam200.sh)
p107_job=$(sbatch --parsable --dependency="afterok:$smoke_job" "${p107[@]}" scripts/slurm_axis_surface_prior_adam200_worker.sh)
student_job=$(sbatch --parsable --dependency="afterok:$smoke_job" "${students[@]}" scripts/slurm_axis_surface_prior_adam200_worker.sh)
