...
- 2026-08-11: `codex/direct-alpha-fast-score` remains report-only; no score code has changed. `reports/direct_alpha_fast_score_feasibility_20260811.md` now deprioritizes direct spatial $\alpha/\iota$ as the next implementation and specifies a local full-dimensional finite-difference route: one formally accepted high-quality center supplies cached axis/surface/QR topology, while 600 endpoints are evaluated by true CUDA query batching with validation/search removed but first-order axis, $s$, point-motion, flux, and $\alpha/\iota$ responses retained. Historical fixed-geometry G3/G4 sign reversals make freezing all geometry invalid. The first implementation gate is gradient-direction agreement with formal endpoints, not proxy-score value agreement; no implementation or timing claim is yet validated.
