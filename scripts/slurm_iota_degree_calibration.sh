...
#SBATCH --job-name=iota-degree-cal
#SBATCH --nodes=1
#SBATCH --ntasks=1
...
output_dir="${OUTPUT_DIR:-$HOME/local_surface_evaluator_runs/iota_degree_calibration_${SLURM_JOB_ID}}"
build_dir="$project/gpu_backend/build_iota_cubic_cuda13"
...
cmake -S gpu_backend -B "$build_dir" -DCMAKE_BUILD_TYPE=Release \
    -DCMAKE_CUDA_COMPILER="$CUDACXX" -DCMAKE_CUDA_ARCHITECTURES=120
...
python scripts/benchmark_score_eval_hinted.py \
    ...
    --device 0 --variants iota_constant iota_cubic --repeats 2 --warmups 1
python scripts/analyze_iota_degree_calibration.py \
