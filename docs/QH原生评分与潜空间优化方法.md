...
**版本日期：** 2026-08-30
**适用实现：** 原生 score ABI 11；QH 目标 $(M,N)=(1,N_{\rm FP})$

当前生产默认使用磁轴安培环流定义 $G$、$48^3$ 的 $\psi$ 拟合网格、三次 $\iota(\rho^2)$ 和 mode 2 严格磁轴续接。QH 优化协议 `qh-flow-screen32-adam200-64d-abi11-v1` 使用 32 候选筛选、64 个新正交中心差分方向、200 步 Adam、$h=0.005$、学习率 0.02 和 FP32 RK4-128。第 6 节中明确标为 ABI-9、ABI-10 或 2 方向的实验是历史证据，其绝对分数和配置均不定义当前方法。
...
实验表明：flow-matching 模型通过 32 候选有限筛选稳定提供高质量起点，并以可逆映射给出潜空间搜索坐标。当前生产配置采用 64 个新正交中心差分方向和 $\beta_1=0.7$ 的 Adam，代表性历史轨迹曾将随机候选提升到 85--93 分；独立完整评估确认高原生分数对应嵌套磁面、小面 QS error 和可被 DESC 显著降低的力残差。309 组同起点结果支持当前潜空间完整配置的高分尾部优势，同时将纯坐标因果效应保留为待完成的同协议消融问题。
...
该边界提供固定成本的局部筛面；严格最大磁面由完整评估确定。表面尺寸分量在有效逆纵横比
...
当前坐标角以弧度计，且 $\psi=\Phi_t/(2\pi)$。ABI-11 沿完整磁轴计算安培环流，并使用

$$
G=\operatorname{sign}(\Phi_t)
\frac{\left|\oint_{\rm axis}\boldsymbol B\cdot d\boldsymbol l\right|}{2\pi}
=\operatorname{sign}(\Phi_t)\frac{\mu_0}{2\pi}
\left|\sum_j I_j\operatorname{Lk}(C_j,C_{\rm axis})\right|.
$$

该定义只计入实际链接选定磁轴的有符号电流。ABI-10 使用 $2N_{\rm FP}\sum_k|I_k|$ 近似链接电流；该近似在出现未链接线圈时失效，现作为历史评分定义保留。报告值统一除以 helicity 向量长度：
...
QH、QA、QP 分别取 $(1,N_{\rm FP})$、$(1,0)$、$(0,N_{\rm FP})$。除全体积 RMS 外还计算外层径向 bin 的 edge RMS。体积权重来自柱坐标 Jacobian、径向射线和采样密度的组合；有效点数量本身不参与权重，因此减少有效点不能直接降低 QS error。
...
快速 score 使用固定 $a=0.05$ 和定长 surface screen；完整评估为每个样本重新选择 $a$ 和 $s$。它先为当前线圈比较多个 $a$，选择验证残差良好的 $s$ 源，再从内向外延续多个候选水平。磁面的最终存在性由标准 Simsopt LS/Newton 与独立 $97\times97$ 密网格验证共同确定。
...
对局部 Lipschitz 的速度场，ODE 流映射是可逆的；同一个模型把积分区间从 $1\to0$ 即得到 $F^{-1}$。当前优化协议使用 FP32 RK4-128；形式反演和高精度景观诊断可使用 RK4-256，并必须记录为不同离散设置。可逆性来自 ODE 流本身，数值误差通过真实线圈反向再正向的闭环误差独立验证。

物理损失主模型使用 4 张 RTX 5090、全局 batch 32768，在恒定学习率 $3\times10^{-4}$ 下训练 30000 步，累计约 47.2 分钟。验证物理 loss 在 30000 步附近达到最低；该 checkpoint 对应实测验证最低点，继续训练出现验证回升。

![物理损失模型的收敛复核](../reports/assets/qh_flow_physical_convergence_20260729/validation_comparison.png)

## 5. 潜空间 SPSA 型梯度与 Adam

本节定义当前 `main` 协议。旧报告中的 2 方向配置已经废弃，只能作为带明确协议标签的历史数值证据，不能用于新实验或默认值推断。
...
每一步随机生成 $K$ 个两两正交方向 $u_j$，令每个方向的 RMS 为 1，即 $\|u_j\|_2=\sqrt D$。用中心差分
...
对随机各向同性正交子空间，该估计在期望上保持正确尺度；多个正交方向降低方向重复和方差。当前正式设置为 $K=64$ 和 $c=0.005$，即每步 128 个 score 端点。每个接受的新中心随后使用严格磁轴续接的完整正式 score 重评。该配置对应已验收的 309 条、每条 200 步轨迹。
...
当前潜空间默认设置为 200 步、$\eta=0.02$、$(\beta_1,\beta_2)=(0.7,0.999)$。这些数值与 64 方向、$c=0.005$ 和 FP32 RK4-128 共同构成一套冻结协议，不能从早期 2 方向扫参中拆出单个参数并继续称为当前默认。标准化原参数空间的历史基线使用不同学习率和半径，只能作为完整配置比较，不能单独证明坐标的因果优劣。
...
2050 个样本由 8 个评分 worker 分摊到 4 卡，评分墙钟为 2600.8 秒，吞吐 0.7882 样本/秒；1024 个随机样本的 flow 解码只占 17.0 秒。原生物理 score 占据当前优化的主要运行时间，flow ODE 解码占比较小。
...
`flux_s` 从长周期候选循环开始计时，与累计的 `surface_screen_s` 包含同一段追踪，因此原始 timing 字段存在有意嵌套，不能逐列相加。剖面说明当前主要成本是磁轴与磁面追踪；100000 点体场、2269 列 $\alpha/\iota$ 线性拟合和 QS 归约耗时较小。

### 6.2 Flow 直接生成的局限

物理损失模型相较旧模型显著改善整体几何与原生 score 分布。第一代模型学到了可行线圈分布和跨 Fourier 阶的相关结构，其单次随机样本仍很少进入 QUASR 的高质量 QH 尾部。当前方法因此使用有限候选筛选提供起点，并使用可逆 Flow 映射提供潜空间搜索坐标。
...
方向。尺度匹配、ODE 闭环和修正 score 的独立对照支持该归因。完整配对统计与证据边界见
...
完整评估独立选择 $s=0.49$，得到体积 $0.06399\,\mathrm m^3$、$\iota=1.6878$、密网格相对残差 $2.65\times10^{-5}$、法向场 P95 $4.30\times10^{-5}$，面 QH error 为 $6.52\times10^{-6}$。DESC 在 50 次迭代上限内保持嵌套，并把归一化力残差均值降到 0.00233。该结果为优化所得几何提供了独立物理质量证据。
...
后续研究集中在三个互补方向。第一，现有 proxy 分类器能以 ROC-AUC 0.934 区分逆追踪 QUASR 潜变量与高斯先验，但与真 score 基本零相关；连续回归器测试 Pearson 约 0.40，却不能识别真实高分尾部。因此 proxy 只有在使用 ABI-11 多任务标签、按高分分层训练和以 top-$k$ 真 score 富集验收后才值得重做。
...
第三，评分器可返回局部近似梯度。axis 候选与最大面选择难以全局反传，且在高分区通常已饱和；coil 工程项容易求导，优先价值集中在固定可行分支后对 volume-QS 及 $\alpha/\iota$ 线性最小二乘求 VJP，再通过可微 flow ODE 回传到潜空间。该近似梯度负责提出方向，ABI-11 正式 score 负责接受和拓扑门控。详细推导、风险和分阶段验收方案见[后续研究方向可行性分析](../reports/qh_future_directions_feasibility.md)。
...
3. flow matching 通过有限筛选提供优质起点，其可逆重参数化对潜空间优化具有实用价值；当前 64 方向、200 步、$\beta_1=0.7$ 的 Adam 协议是默认优化方法。历史优化轨迹已产生经完整物理支线验收的高分 QH 解，其旧方向数只描述对应历史协议。

仍需保留的限制是：快速 score 的固定 $a$ 与有界筛面只适合排序，逐样本最大可行面由完整评估搜索；评分函数含拓扑和候选选择分支，仅具分段光滑性；ABI-10 及更早版本的 volume-QS、landscape、proxy 标签和优化分数均不可与 ABI-11 分数直接比较。
