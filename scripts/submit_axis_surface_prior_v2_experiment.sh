...
p107=(--account=competition --partition=P107-RTX5090 --qos=qos_p107-rtx5090 --cpus-per-task=4 --mem=24G --array=0-3 --job-name=axis-v2-p107 --export=ALL,SHARD_OFFSET=0)
students=(--account=stu --partition=Students --qos=qos_stu_medium_2gpu --cpus-per-task=12 --mem=48G --array=0-1 --job-name=axis-v2-stu --export=ALL,SHARD_OFFSET=4)

sbatch --test-only "${p107[@]}" scripts/slurm_axis_surface_prior_v2_worker.sh
sbatch --test-only "${students[@]}" scripts/slurm_axis_surface_prior_v2_worker.sh
sbatch --test-only --export=ALL scripts/slurm_analyze_axis_surface_prior.sh

mkdir -p "$RUN_ROOT" logs
cp evaluation/axis_surface_contour_prior_balanced_abi11_v2.json "$RUN_ROOT/protocol.json"
p107_job=$(sbatch --parsable "${p107[@]}" scripts/slurm_axis_surface_prior_v2_worker.sh)
student_job=$(sbatch --parsable "${students[@]}" scripts/slurm_axis_surface_prior_v2_worker.sh)
