...
cleanup() {
  ...
  nvidia-smi --query-gpu=index,uuid,utilization.gpu,memory.used --format=csv,noheader,nounits > "$EXPERIMENT_ROOT/gpu_validation_postflight_${shard_index}.csv" 2>/dev/null || true
  exit "$status"
}
...
nvidia-smi --query-gpu=index,uuid,name,utilization.gpu,memory.used,memory.total --format=csv,noheader,nounits > "$EXPERIMENT_ROOT/gpu_validation_preflight_${shard_index}.csv"
...
python "$PROJECT/scripts/run_qh_face_qs_gpu_validation_shard.py" \
