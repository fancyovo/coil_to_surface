...
#SBATCH --job-name=qh-random-adam200
#SBATCH --nodes=1
#SBATCH --ntasks=1
...
worker_index=$(( ${WORKER_OFFSET:-4} + SLURM_ARRAY_TASK_ID ))
...
if nvidia-smi --query-compute-apps=pid --format=csv,noheader,nounits | grep -Eq '[0-9]'; then
  echo "allocated GPU is not idle before Adam200 timing" >&2
  exit 42
fi
nvidia-smi --query-gpu=index,uuid,name,driver_version,utilization.gpu,memory.used,memory.total \
  --format=csv,noheader,nounits > "$gpu_pre"

setsid python scripts/qh_data_space_random_adam_followup.py worker \
  --run-root "$run_root" \
  --worker-index "$worker_index" \
  --device 0 \
  --max-wall-s "${MAX_WALL_S:-13200}" \
  --case-max-wall-s "${CASE_MAX_WALL_S:-3600}" \
  --allow-partial &
child=$!
wait "$child"
child=""
