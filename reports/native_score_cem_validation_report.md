...
## 31. score v3 的 9 小时单种子 CEM 验收

本节记录 2026-07-29 完成的 score v3 长跑。它在上一版基础上增加了两项约束：

1. 候选磁面必须通过 16 个场周期的长期漂移检查；若外层面失败，允许退回较内层面。
2. 同一次 CUDA 体评估同时计算 QH、QA 和 QP 微分残差，并用目标 QH 相对竞争
   helicity 的优势门控总分。

本轮作业 `28753` 使用 $N_{\mathrm{FP}}=3$、4 个基线圈、随机种子
`2026072801`、160 代、每代 160 个候选和 40 个 elite，共评估 25,600 个候选。
作业使用 4 张 RTX 5090，Slurm 墙钟为 8 小时 29 分 9 秒，退出码为 0。结束后的
四张卡均为 0% 利用率且无遗留 GPU 进程。

### 31.1 结论先行

这次长跑得到两个明确结论：

- **score v3 的反作弊门控有效。** 最优候选实际上仍是 QP，而不是 QH；新版 score
  只给它 **3.892/100**，没有再出现“错误位形得到高分”的问题。
- **从零单阶段 CEM 搜索 QH 仍然失败。** 25,600 个候选中没有一个同时满足
  $|\iota|\ge1$ 和 QH 相对优势的最低经验标定阈值。继续按同一分布和同一目标增加
  代数，成功概率很低。

因此，本轮不能解释为“9 小时后得到一个 3.89 分的差 QH”，而应解释为：**搜索始终
没有进入 QH 盆地，score 正确地拒绝了它找到的 QP 退化方向。**

![score v3 的 160 代 CEM 审计](assets/native_score_cem_validation/score_v3_long_run/score_v3_cem_audit.png)
...
| --- | ---: | ---: |
| `ok` | 12,841 | 50.16% |
| `drift_rejected` | 6,729 | 26.29% |
| `no_surface` | 3,298 | 12.88% |
| `no_axis` | 2,605 | 10.18% |
| `flux_rejected` | 127 | 0.50% |

可评分率从第 1 代的 11/160 提高到第 160 代的 107/160，说明 CEM 确实学会了生成更
容易形成轴和有限磁面的线圈。但它没有学到 QH：

| 指标 | 结果 |
| --- | ---: |
| 成功候选的 score 中位数 | 2.345 |
| 全局最高 score | 3.892 |
| QUASR QH score v3 的 P10 | 41.31 |
| 达到 QUASR QH P10 的候选数 | 0 |
| $|\iota|\ge1$ 的候选数 | 47 |
| QH 相对优势不低于坏阈值 0.10 的候选数 | 0 |
| 同时满足 $|\iota|\ge1$ 与 QH 优势不低于 0.10 | 0 |

令单位 helicity 的目标误差为

$$
e_{\mathrm{QH}}=\frac{E_{\mathrm{QH}}}{\sqrt{1^2+1^2}},
$$

竞争误差为

$$
e_{\mathrm{comp}}=\min(E_{\mathrm{QA}},E_{\mathrm{QP}}),
$$
...
$$
q_{\mathrm{adv}}
=\frac{e_{\mathrm{comp}}}{e_{\mathrm{QH}}+e_{\mathrm{comp}}}.
$$

成功候选的 $q_{\mathrm{adv}}$ 中位数只有 0.00626，最大值也只有 0.07329；经验标定的
坏、好阈值分别是 0.10 和 0.30。审计图右下角中所有点都远低于
$e_{\mathrm{comp}}=e_{\mathrm{QH}}$ 对角线，直观说明整个种群都更接近 QA/QP，其中绝大
多数由 QP 竞争项主导。

### 31.3 最高总分候选

最高分出现在第 158 代，其主要诊断为：
...
| $|\iota|$ | 0.9006 |
| 逆纵横比 $a/R$ | 0.01927 |
| 体积 | $0.00342\,\mathrm{m}^3$ |
| 单周期漂移 P95 | 0.00602 |
| 16 周期漂移 P95 | 0.04706 |
| $E_{\mathrm{QH}}$ | 5.6839 |
| $e_{\mathrm{QH}}$ | 4.0191 |
| $E_{\mathrm{QA}}$ | 5.6839 |
| $E_{\mathrm{QP}}$ | 0.01218 |
| $e_{\mathrm{QH}}/E_{\mathrm{QP}}$ | 330.0 |
| QH 相对优势 | 0.00673 |
| QH helicity 总门控因子 | 0.1040 |

该候选刚好擦过 5% 的长期漂移阈值，磁面不大，$|\iota|<1$，而 QP 残差比单位
helicity 的 QH 残差小约 330 倍。它的最小线圈间距只有 3.78 mm、最大曲率约
$432\,\mathrm{m}^{-1}$，工程性质也差。没有必要再对它运行旧 Boozer 面扩展和 DESC：
目标 helicity 已经以两个数量级以上的差距失败，昂贵复核不会改变这一判断。

全种群中 QH 残差最低、相对优势最高的是第 33 代第 94 个候选。它有较稳定且较大的
快速磁面，$a/R=0.0431$，但 $|\iota|$ 只有 0.1288，且
$e_{\mathrm{QH}}/E_{\mathrm{QP}}=28.3$，仍明显是 QP。另一方面，$|\iota|\ge1$ 的候选中
最低 $e_{\mathrm{QH}}$ 为 11.51，对应的 QP 残差仅 0.01233。搜索没有发现 QH 纯度与
足够旋转变换同时成立的交集。

### 31.4 为什么 160 代仍然没有改善

CEM 的平均标准差从 0.346 收缩到 0.038，最优分却只从 1.342 增至 3.892。更重要的
是，逐代最大 QH 相对优势在早期偶尔达到 0.05--0.07，随后随分布收缩下降到约 0.01；
也就是说，优化器提高了轴、磁面可用率和 $\iota$，却主动离开了相对更接近 QH 的方向。

这暴露的不是 QH 诊断公式错误，而是**单阶段标量目标在全 QP 初始盆地中的搜索梯度
不合适**。当前 helicity 总门控保留 0.1 的底，使所有
$q_{\mathrm{adv}}<0.10$ 的候选仍可主要依靠门控前分数排序；在本轮没有任何候选越过
0.10 时，CEM 最容易做的是改善轴、磁面和 $\iota$，而不是发现很窄的 QH 区域。

但简单把该底降为零也不够：本轮相对最像 QH 的候选恰好有很低的 $|\iota|$。若仍使用
单个乘积目标，只会重新产生“QH 相对残差略好但旋转变换退化”的竞争方向。

### 31.5 下一步建议

不建议用相同设置再跑一个更长的随机种子。更可能有效的方案按优先级为：

1. **区分生产 score 与搜索 merit。** 生产 score 继续严格保证非 QH 不得高分；CEM
   内部改用分阶段或分层排序，先达到磁面和 $|\iota|$ 的最低可行条件，再在可行子集中
   主要按 QH 相对优势选 elite，避免各项在一个乘积里互相抵消。
2. **从 QH 支持集启动。** 用 QUASR 的高质量 QH 样本建立混合初始分布，或用已经训练
   的 QH flow model 提供首代，而不是从通用 PCA 的零均值高斯开始。当前实验已经证明
   纯随机分布中的 QH 可行域过稀。
3. **保留 4--5 个基线圈。** 本轮 4 个基线圈的表达能力不是最明显瓶颈；首先应修复
   初始分布和 elite 选择，再比较 4、5 线圈，而不是用更多参数继续稀释随机搜索密度。
4. **把 16 周期漂移作为可行性约束保留。** 它排除了 6,729 个短期看似有面、长期漂移
   过大的候选，且没有造成运行长尾；这部分不需要回退。

### 31.6 验收产物

- [score v3 全候选审计 JSON](assets/native_score_cem_validation/score_v3_long_run/score_v3_cem_audit.json)
- [score、可评分率、CEM 方差与 helicity 分布图](assets/native_score_cem_validation/score_v3_long_run/score_v3_cem_audit.png)
