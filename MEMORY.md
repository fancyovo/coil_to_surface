...
- 2026-08-11 full-density warm-$s$ experiment completed through P107 job `36505`: center-$R_0$-preconditioned CGLS4 solved the full grid48 endpoint matrix in `3.96 ms` with maximum scaled coefficient error `2.56e-5`, but the fixed-$h=0.005$ 300-coordinate complete-score gradient cosine versus fresh endpoint QR was only `0.724`, caused almost entirely by the coordinate component. Coordinate values were not biased low; omitting only its endpoint derivative raised PCGLS4-vs-matching-QR cosine to `0.99990`, but coordinate has real cross-sample screening value and must remain in the formal production score/center acceptance. Psi fit P50 fell from `66.62` to `14.33 ms`, while complete-score P50 only fell from `745.06` to `691.70 ms` (`1.077x`). Production ABI/grid48 QR/optimizer remain unchanged; a future proxy may freeze coordinate at the center but must formally re-score every proposal. Never cite invalid job `36500`. Evidence is the final report sections and `reports/assets/local_psi_warm_start_20260811/`.
