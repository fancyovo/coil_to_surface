...
#SBATCH --job-name=psi-grid-reduce
#SBATCH --nodes=1
#SBATCH --ntasks=1
...
source_dir="${SOURCE_DIR:?SOURCE_DIR must contain the historical holdout worker JSONL files}"
case_dir="${CASE_DIR:-$HOME/local_surface_evaluator_data/volume_score_2000/cases}"
output_dir="${OUTPUT_DIR:-$HOME/local_surface_evaluator_runs/psi_grid_reduction_${SLURM_JOB_ID}}"
build_dir="$project/gpu_backend/build_psi_grid_reduction_cuda13"
...
python scripts/prepare_psi_grid_holdout.py \
    --source-dir "$source_dir" --case-dir "$case_dir" --output "$output_dir/cases.json"
python scripts/benchmark_score_eval_hinted.py \
    --manifest "$output_dir/cases.json" --lib "$lib" --output "$output_dir/benchmark.jsonl" \
    --device 0 --variants baseline psi_grid72 psi_grid64 psi_grid56 psi_grid48 \
    --repeats 2 --warmups 1
python scripts/analyze_psi_grid_reduction.py \
