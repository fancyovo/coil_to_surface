...
- 2026-08-13 active branch is `codex/trajectory-dataset-pilot`. The requested corpus unit draws one joint `(nfp,n_coils)` condition from the exact QUASR QH training distribution, globally scores 32 IID flow latents, selects the best valid start, then runs 200 steps of K64 Adam (`h=0.005`, LR `0.02`, beta `(0.7,0.999)`, FP32 RK4-128). Each completed trajectory must preserve all screened latents/tokens/full scores, every center, all 64 directions/128 endpoint tokens and score components, raw gradient, Adam moments, proposed/applied updates, timings, hashes, and seeds. Six one-GPU streams will write atomically into one root; partial/failed attempts cannot enter `trajectories/`. Pilot target is 20--22 hours, then measure output rate and distributions. Documentation: `docs/qh_adam_trajectory_dataset.md`.
