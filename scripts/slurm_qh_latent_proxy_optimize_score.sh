...
control_scored="${CONTROL_SCORED:-$asset_root/runs/qh_latent_proxy_score_29824/scored_cases.jsonl}"
lib="${SCORE_LIB:-$project/gpu_backend/build_native_score/libstellarator_gpu.so}"
expected_proxy_sha="${EXPECTED_PROXY_SHA:-69797be095b26678a918fee711dd478f61a3474c866ade129029563fa02ee8e4}"
...
run_root="${RUN_ROOT:-$asset_root/runs/qh_latent_proxy_optimized_${SLURM_JOB_ID}}"
...
torchrun --standalone --nproc_per_node=4 scripts/optimize_qh_latent_proxy.py \
  --output-dir "$run_root" \
  --proxy-checkpoint "$proxy_checkpoint" \
  --calibration-summary "$calibration_summary" \
  --flow-checkpoint "$flow_checkpoint" \
  --starts-per-variant "${STARTS_PER_VARIANT:-4096}" \
  --candidates-per-variant "${CANDIDATES_PER_VARIANT:-384}" \
  --steps "${ADAM_STEPS:-300}" \
  --learning-rate "${ADAM_LR:-0.01}" \
