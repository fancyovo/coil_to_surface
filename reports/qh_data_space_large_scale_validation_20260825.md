...
> **纠错权威状态（2026-08-28）：**本文是旧 32 组 2 方向对照的现行协议纠错记录。文中出现的 2 方向数据仅用于说明历史协议错误和结论反转，不构成可复用配置。当前默认协议固定为 64 方向、200 步；纯优化坐标的因果优劣仍未判定。
...
309 条分数计算本身有效；错误发生在**实验归因和报告措辞**。实验同时改变了方向数、潜空间学习率、总步数和随机方向，却把合并效果解释为坐标本身的效果。可发表的坐标结论需要先在独立校准子集上分别选定两边的学习率和差分半径，再在未参与调参的同一批起点上固定方向数、步数和评分预算；每个起点还应重复若干随机方向种子，以免把低方向数估计的方差当作坐标差异。同时报告等步数和等墙钟两种口径。在这项对照完成前，309 条结果只支持两套完整配置的比较。
