...
- Active P107 job `36541` starts from the original `reports/assets/qh_score_adam_start_panel_29960/start_10.json` with zero moments and replaces two-direction SPSA by the 300-D batched coordinate gradient; Adam remains LR `0.01`, beta `(0.7,0.999)`, $h=0.005$, FP32 RK4-128. It requests two RTX5090s, at most 2000 steps, Slurm limit 7:50 and internal wall guard 27300 s; run path is `~/local_surface_evaluator/runtimes/local_score_d1b140f/runs/qh_local_full_gradient_adam/long_start10_20260812`. Wait for user-triggered acceptance after it finishes; do not poll continuously. Historical order-8 result `93.7713@4929` remains on `codex/fourier-mode-8-flow` and is unrelated to this active job.
