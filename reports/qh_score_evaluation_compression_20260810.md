...
优化器相关配置与历史 10000-step 实验一致：QH、$N_{\mathrm{FP}}=4$、3 条基线圈、2 个正交方向的中心差分、扰动 $0.005$、学习率 $0.01$、Adam $\beta=(0.7,0.999)$、FP32 RK4-128、两张 GPU 和跨步流水线。评分器则采用本分支已经验收的新默认：$48^3$ 的 $\psi$ 网格、严格磁轴续接的混合精度核验，以及三次 $\iota(\psi)$。因此下面的时间代表当前路径，不能当作与旧 10000-step 二进制逐项相同的 A/B。
...
每步记录的 score 工作平均为 $2.993\ \mathrm{s}$，flow 工作平均为 $2.222\ \mathrm{s}$；两者通过跨步流水线重叠，不能相加后当作墙钟时间。与历史 10000-step 段记录的 $5.299\ \mathrm{s}$ 迭代计算均值相比，当前路径约快 $22.3\%$，但这个差异同时包含 $48^3$ 网格和删除严格续接中的 FP64 重放，不能归因于 $\iota$ 修改。单独的 69 样本 A/B 已经证明：三次 $\iota$ 本身没有可测的耗时代价。
...
2. 两方向中心差分中，如果从第 301 步开始固定一条方向为上一次实际接受的 Adam 更新方向，是否比每步重新随机两条正交方向更有效。
...
再按原来的两方向估计器构造 Adam 输入。若某一步没有接受更新，则不存在新的实际更新方向，程序继续保留最近一次被接受的更新方向。这个细节后来成为实验组尾部停滞的直接原因。
...
1. **新 score 的长跑有效。** 两条新随机方向的现行默认方法从 $85.5157$ 提高到 $92.6818$，并在第 994 步刷新最佳值；1000 步范围内没有无效中心或时间门拒绝。
2. **朴素复用上一更新方向没有额外增益。** 它在分叉后的过程均值和最终最佳值上都略差，并在第 924 步后触发连续 77 步停滞。
3. **生产默认保持不变。** 仍使用每步两条新的随机正交方向；本次实验开关默认关闭。
