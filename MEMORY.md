...
- 2026-08-12 branch `codex/direct-alpha-fast-score` validated query-batched local gradients and true single-RTX5090 execution. Full-coordinate Adam reached `93.6304` but cost `15.352 s/step`; BFGS failed. Fresh `K=64` orthogonal directions with LR `0.02` were the best reduced estimator. Accepted evidence commit `0552264`; report `reports/direct_alpha_fast_score_feasibility_20260811.md`; production SPSA remains unchanged.
- 2026-08-12 commit `c9213aa` made flow+score truly single-GPU and cross-iteration prefetched each accepted center plus 128 endpoints in one FP32 RK4-128 batch. No-pipeline/pipeline smoke was `7.105/~5.52 s/step` (`1.287x`). Job `36689` completed 200/200 accepted updates: `85.5045 -> best 93.0961` at step 197, `1147.79 s` wall, steady P50/P95 `5.463/5.878 s`; independent history-free score `94.6202` mainly reflects its higher surface component (`97.56` versus `82.59`).
- Complete evaluation selected sample-specific `a=0.08,s=0.49`; `s=0.24/0.36/0.49` passed standard LS/Newton with increasing volume, while `s=0.64` was the neighboring GPU-ray fixed-budget failure (`170732 < 180000`). Selected volume is `0.066596 m^3`, iota `1.46030`, dense relative L2 `1.392e-5`, normal-field P95 `1.979e-5`, and face QA/QH/QP `4.745e-3/1.246e-6/4.814e-3`. Poincare is nested and direct/DESC colored contours are QH. DESC stayed nested and reduced normalized force mean/P95/max from `1.135/1.786/3.581` to `1.092e-3/2.476e-3/8.439e-3`, but hit its 50-step limit (`optimizer_success=false`). All eight DESC PNGs passed delivery validation. Artifacts: `reports/assets/local_gradient_single_gpu_200_20260812/` and `reports/assets/local_gradient_single_gpu_200_full_eval_20260812/`.
- Full evaluation requires query-batch library hash `7834a88d...`; shared `build_mixed` was stale and lacked `sgpu_trace_axis_samples`. Jobs `36699/36701/36703/36705` ran no physics and are invalid. `scripts/slurm_smoke_native_score.sh` now accepts explicit `PROJECT`/`SCORE_LIB` pins.
