...
- 2026-08-12 branch `codex/direct-alpha-fast-score` validated the single-RTX5090 logical 600-query local-gradient API and full 300-D Adam. Job `36541` completed 1739 fully valid steps, raising formal score `85.5045 -> 93.6304218` while averaging `15.352 s/step`; full evidence and the experimental library hash are in `reports/direct_alpha_fast_score_feasibility_20260811.md` and `reports/assets/local_query_batch_gradient_20260812/`. Production SPSA remains unchanged.
- 2026-08-12 optimizer sweep accepted in `8908a8a` found no benefit from full BFGS: only 3/40 and 2/40 formal updates were accepted at step-100/1250 anchors. Fresh random orthogonal directions were effective: `K=64`, Adam LR `0.02` reached `93.2979847` in 300 steps at `6.514 s/step`, outperforming `K=32` and LR `0.01/0.03` in this one-start/one-seed test. Treat it as the next experimental candidate, not a production default. Jobs `36605/36606/36607` completed cleanly with no residual processes; see the report's final section and `reports/assets/local_gradient_optimizer_sweep_20260812/`.
- 2026-08-12 timing clarification for that `K=64` run: `6.514 s/step` is a measured two-RTX5090 role split (GPU 0 FP32 RK4-128 flow, GPU 1 native score), not a measured one-physical-GPU end-to-end run. The stages are sequential: flow used `2.452 s` (37.6%) and native scoring `4.058 s` (62.3%). Across 300 steps, P50/P95/max were `6.562/6.647/6.719 s`, SD `0.110 s`; there was no long tail. The 128-query local score took `1.697 s`; surface trace, flux, volume field, alpha, and QS means were `0.526/0.528/0.181/0.405/0.047 s`. The clean FP32 Biot--Savart kernels delivered about `20.9` effective conventional TFLOP/s for flux and `27.4` for volume B+gradient, excluding one `rsqrt` per point-segment interaction. Surface trace reached only about `1.27` effective TFLOP/s because each line is a serial RK4 chain. Psi uses four-step matrix-free PCGLS here, not Householder QR; feature construction (`0.246 s`) dominates its `0.285 s` stage while the solve is about `0.027 s`.
- 2026-08-12 commit `c9213aa` replaced the artificial two-GPU role split with a true single-GPU flow+score path and cross-iteration flow prefetch: each accepted center is decoded together with the next 128 endpoints as one 129-sample FP32 RK4-128 batch. Single-RTX5090 smoke job `36688` completed 4/4 valid steps; steady cache-hit steps 2--3 took `5.565/5.476 s` versus the old two-card sequential mean `6.514 s`, with identical first-step score/update for seed 6401. Formal 200-step job `36689` is running from the exact original start with `K=64`, LR `0.02`; after it completes, its best must receive the fixed full physical evaluation.
