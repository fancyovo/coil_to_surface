...
- On 2026-08-08, the public `StellCoilOpt` documentation repair was published
  through commits `93cbdb1` and `4062b16` on `main`. It adds the advisor-deck
  runtime benchmark and explanation, fixes all diagram text bounds, uses the
  implemented vector relation `B ~= grad(psi) x grad(alpha)`, connects the
  `alpha+nu` full-validation branch after the alpha/iota fit, and removes the
  obsolete branch label. Markdown math was rewritten with supported multiline
  syntax and automated delimiter/link checks. Local compileall, privacy checks,
  image inspection, and all 63 tests pass. P107 job `33812` then completed
  (`0:0`, 45 s) against exact commit
  `4062b163cea12db636e2ccdc37b1da5d650f2ea5`: the pinned source archive SHA-256
  was `a51180d413a1b95e8ba63fad618fefa2bb421e87ba366df86404fceed5b5e1a9`,
  CUDA 13.0 / GCC 13.3 built the Release architecture-120 shared library on an
  RTX 5090, and the packaged example smoke returned `status=ok`, score
  `86.3036575076`, and completed stage 8. The only compiler diagnostic is an
  unused-function warning in `coil_field.cu`. Jobs `33804/33807` are invalid
  pre-build launcher/network attempts; `33809` built successfully but stopped
  after linking because the remote venv lacks pytest. No CUDA verification job
  remains; the temporary local/remote transfer, script, and log files were
  removed. The source project was unchanged apart from this memory entry.
...
- Active P107 job `33799` started on idle `anode03` at 2026-08-08 10:31
  ...
  at 20:31.
- On 2026-08-07, the publication audit found no passwords, tokens, private
  keys, or credential-bearing URLs, but the tracked tree and its history contain
  extensive personal/infrastructure identifiers and machine-specific paths, so
  the existing repository must not be published directly. A separate sanitized
  candidate now exists at `../opensource_staging`: it contains only curated
  evaluator/CUDA/flow/optimization/full-validation code, focused tests, a
  redistributable example, generated method schematics, a concise README, and a
  method-only technical document. It excludes datasets, weights, run artifacts,
  benchmarks, remote launchers, and exploration reports. Its 60 local tests and
  Markdown/link/privacy checks pass; CUDA compilation was not available on the
  local machine. On 2026-08-07 the user selected the name `StellCoilOpt`, the
  Python distribution name `stellcoilopt`, and the MIT license. The sanitized
  repository was published from `../opensource_staging` as a new root commit
  `9d6fdb4` on `https://github.com/fancyovo/StellCoilOpt`, with `main` tracking
  `origin/main`; the commit uses the account's public no-reply identity rather
  than the machine's private email. The source project remained read-only for
  the curation apart from these brief memory updates.
- On 2026-08-07, full physical evaluation of the 2000-step beta1 `0.7` best
  ...
  `main` were atomically synchronized through remote-sync record `054720d`.
  All jobs are complete, the queue is empty, and no project process remains.
- On 2026-08-06/07, the user requested a compiled Chinese LaTeX/Beamer advisor
  presentation under the isolated root
  `presentations/advisor_report_20260807/`. Except for that root and brief
  `MEMORY.md` updates, all existing project content is read-only for this task.
  The numerical example is fixed to the reported 600-step beta1 `0.7` run's
  best iteration `574`: source
  `reports/assets/qh_score_fast_beta1_long_20260806/runs/beta1_0p7/best.json`,
  native score `92.3826396812`, SHA-256
  `929835799be8f5ec0a75500bc9c52b9d9538659db4a93763d1ae0cdab98ecdd1`.
  This identity must not change if later evaluations become newer.
- On 2026-08-07, the advisor presentation was revised after detailed user page
  review into a 24-page, 9-point expert-audience deck. The former evaluator
  data-flow and axis-diagnostic slides were removed; the magnetic-axis,
  fitted-$s$, candidate-surface, $\alpha$, benchmark, future-direction, and
  conclusion slides were shortened or rewritten as requested. The compiled
  PDF is `presentations/advisor_report_20260807/build/main.pdf`; the matching
  13-image fallback document is
  `presentations/advisor_report_20260807/advisor_report.md`; Beamer source is
  `presentations/advisor_report_20260807/source/main.tex`. XeLaTeX compilation
  and 24-page raster/contact-sheet inspection pass with no overfull boxes,
  missing images, placeholders, or visible text overlap. The user explicitly
  rejects filler, ceremonial language, unexplained internal jargon, visible
  hashes, abnormal labels such as "frozen sample", and low-information section
  splitting.
- The presentation's fitted-$s$ figure now uses closed two-dimensional grid
  contours selected against the existing ray references; all 20 curves across
  four toroidal sections and five levels have measured endpoint gaps between
  zero and `5.55e-17 m`. The colored Poincare points remain the previously
  computed 5120-point native-GPU trace; no interpolated or newly traced points
  were claimed. The local-score landscape slide continues to use the existing
  31 measured amplitudes, which were already concentrated near zero and sparse
  outside; every measured vertex is now drawn explicitly so the steep
  coil-space response is not hidden by an unmarked polygonal line. A finer
  35-amplitude GPU rerun and combined plotting launcher are prepared only under
  `presentations/advisor_report_20260807/`; they were not submitted because the
  mandatory first WSL preflight command failed with
  `Wsl/Service/E_ACCESSDENIED`. No presentation Slurm job was created.
- The presentation's current-score 1024+1024 measurement is complete. QUASR
  has `849/1024 = 82.9%` status-ok, score median/mean `82.4045/66.8739`, and
  `570/156` cases at score at least `80/90`; failures are `no_surface=113`,
  `flux_rejected=51`, and `no_axis=11`. Random flow has `62.8%` status-ok,
  score median/mean `17.1194/30.1108`, `6/0` cases at score at least `80/90`.
  Global-search wall time median/P95/max is `2.8468/5.1460/11.5282 s`.
  Among 1013 eligible paired cases, strict magnetic-axis continuation gives
  `0.9694/1.2214/1.6297 s` and `2.99x` paired median speedup. On two RTX 5090s,
  global/hint throughputs are `0.62/2.05 sample/s`. Global/hint stage medians
  in seconds are coil `0.0232/0.0232`, axis `2.1213/0.2485`, s+psi
  `0.4319/0.4317`, surface `0.1850/0.1863`, and alpha+QS `0.0647/0.0648`.
- The current-score landscape rerun covers three centers, four directions,
  31 signed amplitudes, and 1095 unique cases. Relative to independent random
  coil-space directions, latent directions have median drop-5 width ratio
  `7.70995` and are wider in `12/12`, median physical-radius ratio `3.51198`,
  and median second-derivative roughness ratio `0.298969` with `9/12` smoother.
  Relative to transported flow tangents, the median drop-5 width ratio is only
  `0.93543`; interpret the useful geometry as learned correlated directions,
  not coordinate rescaling. The selected-sample optimizer moves score
  `85.5016048 -> 92.3826397`; component start/best pairs are axis
  `97.4246/96.9100`, psi `97.5496/99.1108`, surface `78.9472/83.1418`,
  coordinate `86.3412/90.8427`, volume-QS `80.8723/95.5569`, iota `100/100`,
  and coil `68.8622/65.6024`.
- Presentation job `33280` completed all required numerical work before its
  final plotting process failed only because the installed Matplotlib version
  requires `Axes.boxplot(tick_labels=...)` rather than `labels=...`. Local
  postprocessing with that compatibility fix succeeded. Its native GPU
  Poincare route traces 40 seeds for 32 periods, then batches four section
  returns, producing 5120 points (1280 per section) through `CoilFieldGpu`.
  No presentation job or process remains active. Retain these implementation
  lessons: set Slurm `--chdir`; use the batched CUDA tracer rather than Simsopt's
  sequential adaptive CPU field-line route; include `gpu_backend/python` on
  the import path; do not retain unused dense contour/ray scans; and use the
  Matplotlib-compatible `tick_labels` keyword. The cluster's
  `sbatch --test-only` start estimates are inaccurate even when jobs start
  immediately, so use it only to validate submission syntax/resources.
- On 2026-08-07, continuation job `33166` was accepted after completing all
  2000 iterations with `status=ok`: final score `92.9307316830`, best score
  `93.0409329399` at iteration `1945`, 1973 applied Adam steps, and
  `11235.49 s` total wall time. Relative to the 600-step running best it gained
  `0.658293`; the final 100/200/400-step running-best gains remained positive,
  so returns are diminishing but strict convergence is not established.
  Frozen artifacts and the readable acceptance are in
  `reports/assets/qh_score_fast_beta1_0p7_continue2000_20260807/` and section 18
  of `reports/qh_score_throughput_and_continuous_surface_plan.md`; they are
  visible in the normal feature worktree through commit `e48358d`, while main
  integration is recorded by `591146f` and remote `main` by the subsequent
  memory commit `1e7a626`. Its later full alpha+nu/LS/Newton/DESC evaluation
  is accepted in section 19 and the first snapshot item above. Per the explicit
  presentation freeze, the advisor deck must continue to use iteration `574`
  / score `92.3826396812`. Initial job `33163` remains invalid; commit
  `3a7a2bb` fixed its resume-axis-hint bug without relaxing score consistency.
...
- On 2026-08-06, commit `6582daa` accepted the three same-start 600-step,
  ...
  learning rate `0.01`, perturbation `0.005`, seed `20260804`, pipelined FP32
  RK4-128, two central-difference directions, strict axis continuation,
  continuous score, and only score finite differences. Their best scores were
  `92.18260/92.38264/92.36607` at steps `595/574/580`; beta1 `0.7` is the
  preferred parameter for this fixed start and budget, with beta1 `0.9` only
  `0.01657` lower. None is demonstrably converged: their final 100-step
  running-best gains were `0.21062/0.14218/0.19923`. Runtime was
  `53.9--55.6 min` per 600-step two-GPU job, mean step wall was `5.27--5.40 s`,
  and no optimizer, score process, or GPU allocation remained afterward.
  This was native-score acceptance only; no alpha+nu, LS/Newton, or DESC full
  evaluation was run. Detailed evidence is section 16 of
  `reports/qh_score_throughput_and_continuous_surface_plan.md`, with frozen raw
  assets in `reports/assets/qh_score_fast_beta1_long_20260806/`.
- The forward score optimizer defaults remain pipelined FP32 RK4-128 with two
  central-difference directions, while `--flow-steps`/`FLOW_STEPS` and
  `--directions`/`DIRECTIONS` remain override interfaces; source/README commit
  `5fd7fc3` introduced them. The first beta1 jobs `33032/33033/33034` are
  invalid and were cancelled because a comma in Slurm `--export` truncated
  `SCORE_GPUS=0,1` to one worker. Commit `d68905f` accepts colon-separated
  `SCORE_GPUS=0:1` and normalizes it after export. Job `33043` exited before
  optimization because its allocated GPUs failed the idle gate and was
  superseded by valid job `33049`. Never use the non-v2 root or jobs
  `33032--33034/33043` in beta1 analysis. Valid job `33042` emitted only a
  post-result Python semaphore cleanup warning; jobs `33049/33044` had empty
  stderr, and no numerical/CUDA failure occurred.
...
- Reports must use plain language as the primary narrative, not dense internal
  ...
  and raw summaries.
- Advisor presentations target domain experts and should maximize useful
  technical content: use compact typography, keep each major workflow step to
  one slide or at most two including its figure, and remove filler, ceremonial
  prose, provenance hashes, internal bookkeeping labels, and unexplained
  shorthand. Maintain an image-rich Markdown document with the same logical
  order as a fallback presentation format.
