...
def append(report: Path, evaluation: Path, selection: Path) -> None:
    ...
    lines = [
        ...
        f"选择记录：`{selected['case_file']}`；完整评估摘要：`{evaluation.resolve()}`。", "",
        "| 阶段 | 结果 |", "|---|---|",
        f"| source-psi | `{source.get('selected', source).get('a_m', source.get('selected_a_m', 'see summary'))}`，完整候选见 `{summary['artifacts']['source_psi_selection']}` |",
        ...
        f"| DESC | 状态 `{downstream.get('status')}`，结果见 `{summary['artifacts']['desc']}` |",
