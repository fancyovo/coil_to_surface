...
| 样本 | 稠密相对 L2 | 法向场比 p95 | QA | QH `(1,+1)` | QP | `|B|` 范围 (T) | Poincare |
|---|---:|---:|---:|---:|---:|---:|---:|
...
两例正式计算从 19:14:50 持续到 20:33:57，共 79 分 07 秒。主要延迟来自显式串行
调度：14 个相互独立的 `s` 候选形成两条 `afterany` 链，实测墙钟合计 52 分 37 秒。
按相同作业时长回放，四卡并行约需 13 分 12 秒，六卡并行约需 8 分 47 秒。两例下游
CPU 作业分别耗时 6 分 17 秒和 5 分 24 秒，也可跨样本并行。

第一次不兼容 GPU 库提交到第二例完成共 88 分 56 秒；结果下载、逐图检查、报告生成与
交付约用 30 分钟，形成接近两小时的交互墙钟。串行设置只影响运行效率，冻结物理结果和
数值结论保持不变。完整评估启动器现已默认并行，串行模式必须提供并记录资源限制理由；
P107 与 Students 候选池可共同使用六张卡。逐作业时间戳与回放数据见
[调度审计 JSON](assets/axis_surface_prior_balanced_v2_adam200_20260901/full_eval_scheduling_audit.json)。
