...
- 2026-08-11 correction/current experiment: jobs `36448/36458` tested reduced-density endpoint QR, not the requested full-density warm-start iterative $s$ solve, so their failed total-score gradients do not reject that route. Commit `99556dd` adds an isolated grid48 center-to-endpoint warm-CGLS benchmark. P107 job `36468` failed before compilation because its script selected system CUDA 12 for `sm_120`; it produced no matrices/results and left the GPU idle. The corrected rerun must use explicit CUDA 13 and compare 0/1/2/4/8/16/32/64 iterations on four coordinate directions at fixed `h=0.005`. If the matrix layer passes, compare identical score endpoints while continuing the axis, refitting $s$, and fixing at most scalar $s_0$; never reuse fixed-axis/fixed-$s$ variants. ABI/default score remain unchanged; report is `reports/direct_alpha_fast_score_feasibility_20260811.md`.
