...
- 2026-08-11 full-density warm-$s$ experiment completed through P107 job `36505`: center-$R_0$-preconditioned CGLS4 solved the full grid48 endpoint matrix in `3.96 ms` with maximum scaled coefficient error `2.56e-5`, but the fixed-$h=0.005$ 300-coordinate complete-score gradient cosine versus fresh endpoint QR was only `0.724` (relative error `0.733`), mainly because the coordinate-component cosine was `0.099`. Psi fit P50 fell from `66.62` to `14.33 ms`, while complete-score P50 only fell from `745.06` to `691.70 ms` (`1.077x`). It is a valid local linear-solver result but fails as a formal-score gradient replacement; production ABI/grid48 QR/optimizer remain unchanged. Never cite invalid pre-fix job `36500`. Evidence is the final section of `reports/direct_alpha_fast_score_feasibility_20260811.md` and `reports/assets/local_psi_warm_start_20260811/`.
