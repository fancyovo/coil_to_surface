...
> **废弃优化协议提示（2026-08-28）：**本报告第 15 节以后包含 2026-08-06 至 08-10 的 2 方向实验和当时的默认值判断。这些数值仅是历史协议证据；其中关于“生产默认”“推荐 2 方向”的表述已经全部失效。当前 `main` 使用 32 候选筛选、64 方向中心差分、200 步 Adam、$h=0.005$、学习率 0.02 和 FP32 RK4-128。报告中的评分器、磁轴续接、连续磁面与完整物理验收结果仍按各自记录的代码和协议解释。
...
## 15. 流水线、RK4 步数和差分方式的 200 步矩阵验收（废弃 2D 历史协议）
...
在这组废弃历史矩阵中，RK4-64/中心 2 方向的 flow 占 121.72 s，原生 score 占 1365.37 s。它用一张 GPU 在 25.34 min 内达到 91.4682；历史 job `32804` 用四张 GPU 在 31.04 min 内达到 91.8749。RK4-128/中心 2 方向用 27.63 min 达到 91.7463，并被当期报告判断为质量/速度折中；该判断现已失效。

本轮单种子验收支持以下决策：

1. **当期接受跨迭代 flow 流水线。**它把同配置 flow 耗时近乎减半，并保持历史优化结果；流水线机制仍被当前实现使用。
2. **当期拒绝单边 4 方向。**它与中心 2 方向成本相同，但三个 RK4 档位都明显更差。
3. **已废弃的当期推荐：RK4-128 + 中心 2 方向。**该配置不得用于新实验；当前协议使用 RK4-128 + 中心 64 方向。
4. 本轮只有一个固定起点，所得方向数判断从未构成跨分布结论，现仅保留为历史矩阵记录。

原始 manifest、200 步 `history.jsonl`、best、GPU 前后状态、stderr 和分析表均保存在 [验收资产目录](assets/qh_score_fast_optimizer_matrix_20260806/)。

## 16. 600 步 Adam 一阶动量扫参验收（废弃 2D 历史协议）

### 16.1 实验比较的是什么

本轮历史扫参固定使用上一节当期选出的配置：FP32 RK4-128、跨迭代 flow 流水线、2 个正交随机方向的中心差分、连续磁面 score 和严格磁轴续接。三组实验使用同一个 latent 起点、随机种子 `20260804`、学习率 $\eta=0.01$、扰动尺度 $h=0.005$ 和 $\beta_2=0.999$，唯一改变的是一阶动量 $\beta_1=0.5,0.7,0.9$。每组运行 600 步，并使用两张 RTX 5090 并行计算原生 score。该方向数配置已经废弃。
...
三组累计 flow 墙钟为 673--740 s，原生 score 墙钟为 2474--2494 s。前者约占总墙钟 21%--22%，后者约占 75%--77%；其余是初始化、日志和少量调度开销。在这组历史 RK4-128、2 方向中心差分和双 score GPU 配置下，瓶颈回到原生 score。平均每步为 5.27--5.40 s，P95 为 5.50--5.87 s，没有发现极端长尾。

本轮历史验收支持在该冻结 2D 配置内优先使用 $\beta_1=0.7$。当前 64D 协议同样使用 $\beta_1=0.7$，其当前地位由现行协议验证建立，不能由本节的 2D 扫参单独外推。
...
## 19. 历史 2D 轨迹 2000 步 best 的完整物理评估
...
## 21. 历史 2D 轨迹第 4341 步 best 的完整物理评估
...
## 22. 历史 2D 轨迹从 5000 步继续到 10000 步
