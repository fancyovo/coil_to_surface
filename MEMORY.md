...
- 2026-08-11 correction/current experiment: jobs `36448/36458` tested reduced-density endpoint QR, not the requested full-density warm-start iterative $s$ solve, so their failed total-score gradients do not reject that route. P107 job `36469` used CUDA 13 and successfully froze the center plus eight fixed-$h=0.005$ endpoint grid48 augmented matrices (about 4.5 GB total) under `~/local_surface_evaluator/runs/local_psi_warm_start_20260811_v2/snapshots`, but a one-character `warmcgls` iteration-parser bug stopped it before numerical benchmarking. The fix reuses those snapshots and must compare 0/1/2/4/8/16/32/64 warm iterations without regeneration. If the matrix layer passes, compare identical score endpoints while continuing the axis, refitting $s$, and fixing at most scalar $s_0$; never reuse fixed-axis/fixed-$s$ variants. ABI/default score remain unchanged; report is `reports/direct_alpha_fast_score_feasibility_20260811.md`.
