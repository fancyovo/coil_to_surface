...
- $\psi$ 拟合使用 $48\times48\times48$ 网格和 4000 个独立验证点。
...
在空闲 RTX 5090 上，69 个严格续接样本使用 `48^3` 网格时单次 score 的 P50/P95 为 0.665/0.913 秒；精确磁轴提示并使用 mode 2 时为 0.551/0.730 秒，人为加入 $10^{-3}$ 提示偏移时为 1.010/1.392 秒。独立样本的全局磁轴搜索成本更高，不能用严格续接数字代替。
