...
#SBATCH --job-name=qh-random-score
#SBATCH --nodes=1
#SBATCH --ntasks=1
...
run_root="${RUN_ROOT:-$asset_root/runs/qh_random_score_pool_${SLURM_JOB_ID}}"
...
python scripts/evaluate_qh_random_score_pool.py \
  ...
  --count "${RANDOM_COUNT:-4096}" \
  --seed "${RANDOM_SEED:-20260805}" \
...
python scripts/evaluate_qh_random_score_pool.py \
  ...
  --lib "$lib" \
  --gpu-ids 0,1,2,3 \
  --timeout-s "${SCORE_TIMEOUT_S:-10800}" &
children+=("$!")
wait "${children[0]}"
children=()
