...
第二类是配对优化。实验从 8 条轨迹的第 1、50、150 和 200 步选取跨条件中心，覆盖 $N_{\mathrm{FP}}=3,4,5$ 和 2--5 根基本线圈。Flow 分支从保存的潜变量出发；原参数分支从同一来源线圈的标准化 Fourier token 出发。Flow 反演再正演使两侧初始正式分数最多相差 0.0188。两侧均运行 100 步、每步两个正交方向、中心差分、$\eta=0.01$ 和 $(\beta_1,\beta_2)=(0.7,0.999)$；有限差分半径在潜空间为 0.005，在标准化原参数空间为 0.0025。两个坐标的单位不同，因此该实验比较各自稳定工作配置下的实际优化效果。

![图 8：8 个匹配中心在 Flow 潜空间和标准化原参数空间中的 100 步优化。上排给出相对各自起点的增益，下排给出成对最优增益与墙钟成本。](assets/flow_pairs_current_20260824/analysis/flow_vs_data_optimization.png)

| 成对指标 | Flow 潜空间 | 标准化原参数空间 |
|---|---:|---:|
| 100 步最优增益 P50 | 1.872 | 0.000 |
| 共同 158.1 秒预算内增益 P50 | 1.831 | 0.000 |
| 100 步墙钟 P50 | 305.8 s | 202.8 s |
| 最终胜出的匹配中心数 | 6/8 | 2/8 |

Flow 最优分减去原参数最优分的 P10/P50/P90 为 $-1.977/0.222/1.260$。图 8 左上显示 Flow 的中位增益持续到约第 50 步，原参数空间有两个大幅改善个例，同时至少一半中心停在起点。右上把预算限制在所有运行都覆盖的 158.1 秒，Flow 中位增益仍为 1.831。右下说明代价：Flow 每步需要 RK4-128 解码，100 步墙钟中位数是原参数空间的 1.57 倍；一个低分中心因反复可行性回退形成 13.7 分钟长尾。该组结果支持“Flow 提高了跨中心优化成功的中位稳健性”，同时给出两个明确反例和额外解码成本，因此证据不支持逐个中心或固定迭代下的普遍优势。
...
| 当期三模式、配对优化与 landscape 动态库 | SHA-256 `50877cdb7afa79433b2c337ac02953ac288b772a5c0cfc4658ec688a1d1791f5` |
...
- [当期 8 组配对优化汇总](assets/flow_pairs_current_20260824/analysis/summary.json)
