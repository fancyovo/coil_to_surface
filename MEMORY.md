...
- 2026-08-11 full-density warm-$s$ experiment: P107 jobs `36474/36476` completed the center plus eight fixed-$h=0.005$ endpoint grid48 matrix comparison using the snapshots under `~/local_surface_evaluator/runs/local_psi_warm_start_20260811_v2`. Two CGLS iterations cost P50 `1.951 ms`, kept every endpoint's training physical residual within `1.038x` endpoint QR, and had maximum scaled coefficient error `6.08e-4`; 8 iterations cost `5.844 ms` and reduced the worst residual ratio to `1.011`. Thus the matrix layer passes. The active next gate is an identical-endpoint complete-score comparison at 0/2/4/8/16 iterations while continuing the axis, rebuilding/refitting full-density $s$, and rerunning every downstream stage; never reuse fixed-axis/fixed-$s$ variants. ABI/default score remain unchanged; report is `reports/direct_alpha_fast_score_feasibility_20260811.md`.
