...
for _ in {1..60}; do
  ...
  done < <(nvidia-smi --id="$gpu_selector" --query-gpu=utilization.gpu,memory.used --format=csv,noheader,nounits)
  if nvidia-smi --id="$gpu_selector" --query-compute-apps=pid --format=csv,noheader,nounits | grep -Eq '[0-9]'; then idle=0; fi
  if (( idle )); then
    ((idle_streak += 1))
...
python scripts/probe_qh_physical_gradient_bias.py \
  --run-dir "$run_dir" \
  --iteration "$iteration" \
