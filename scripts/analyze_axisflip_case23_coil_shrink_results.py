...
def main() -> None:
    ...
    axes[0].set(xlabel="Per-coil-anchor scale", ylabel="ABI-11 total score")
    axes[0].legend()
    axes[1].plot(scales, radius, "o-", label="effective coil radius")
    axes[1].plot(scales, axis_mean, "s-", label="mean coil-axis distance")
    axes[1].set(xlabel="Per-coil-anchor scale", ylabel="Distance [m]")
    axes[1].legend()
    axes[2].plot(scales, margin, "o-", color="#27824a")
    axes[2].axhline(0.0, color="#555555", linewidth=1)
    axes[2].set(xlabel="Per-coil-anchor scale", ylabel="Minimum tube margin [m]")
    for axis in axes:
        axis.grid(alpha=0.22)
    figure.suptitle("Case 23 per-coil-anchor similarity-shrink scan")
