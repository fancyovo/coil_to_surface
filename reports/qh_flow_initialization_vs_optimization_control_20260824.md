...
现有结论应该收紧为：**Flow 已被明确证明能提供很强的 QH 生成先验，从而通过有限批量筛选稳定得到好初值；但没有证据表明 latent 空间本身是比标准化线圈参数空间更好的局部优化坐标。**

更准确地说，不是“Flow 每次生成的都是好线圈”。本次 1536 个 Flow 候选的分数中位数只有 9.543，有效率为 61.2%。真正稳定的是“生成 32 个、按正式评分取最高”这一过程：48 组条件全部找到有效起点，起点分数中位数为 77.339，44/48 超过 50 分。作为对照，只保留每个参数的均值和方差、但不保留参数间相关性的独立高斯先验，在 48 组中有 23 组连一个有效起点都找不到，32 选 1 的起点分数中位数只有 0.423。

此前的同起点实验则隔离了“优化坐标”本身：让 latent Adam 和原参数 Adam 从同一个 Flow 生成线圈出发，原参数空间的 100 步增益中位数为 8.664，latent 空间为 7.676；原参数空间赢 22/32 组，且 latent 路径耗时为前者的 1.514 倍。因此，当前唯一得到强证据支持的正向作用是 **Flow 先验和初始化**，不是 latent 局部优化。
...
这不等于证明 latent 优化在所有区域都无用，也不否定先前局部 landscape 中观察到的个别平滑方向。它只要求不再把局部案例写成普遍的优化增益。一个合理的后续主线是“Flow 负责生成和筛选，解码后直接在原参数空间优化”，但在替换现有默认流程前仍应做更大规模的同起点验证。
