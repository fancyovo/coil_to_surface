...
**版本日期：** 2026-08-03  
**适用实现：** 原生 score ABI 9；QH 目标 $(M,N)=(1,N_{\rm FP})$
...
实验表明：直接从训练后的 flow-matching 模型采样仍不能稳定产生高质量 QH 尾部，但该可逆流将原始 Fourier 参数空间重参数化为更易搜索的潜空间。采用四个正交反向差分方向估计梯度，并以低动量 Adam 更新，可以在固定条件下将随机候选提升到 85--93 分；独立完整评估确认高原生分数对应嵌套磁面、小面 QS error 和可被 DESC 显著降低的力残差。
...
评分器在轴周围圆盘内对 $R,Z$ 作均匀 Cartesian 网格采样，并在一个场周期内均匀采样 $\phi$；默认 $80\times80\times80$ 网格在圆盘和内孔筛选后形成数十万行。CUDA 生成场值和设计矩阵，cuSOLVER 以 FP32 QR 求带 $10^{-6}$ 谱正则的线性最小二乘。另用 4000 个面积均匀随机点验证
...
对预设 $s$ 水平，从磁轴沿极角射线求解 $s(R,Z,0)=s_j$，再追踪一个场周期和 16 个长周期。漂移使用物理距离归一化

$$
d_s\simeq\frac{|s_{\rm end}-s_j|}{|\nabla s|},
$$

从而避免不同 $s$ 标度造成的虚假差异。默认候选从 $10^{-3}$ 延伸到 $0.81$。选择不是“严格最大面”：要求有稳定长追踪并优先选择较大的连续水平；表面尺寸分量在有效逆纵横比

$$
\epsilon_a=\frac{a_{\rm eff}}{R_{\rm major}}
$$

达到 0.03 后饱和，防止仅靠继续增大磁面刷分。
...
符号取自边界环向磁通。ABI 9 同时修复了该 $2\pi$ 约定和全局电流反号约定。报告值统一除以 helicity 向量长度：
...
对随机各向同性正交子空间，该估计在期望上保持正确尺度；相较单对 SPSA，四个正交方向降低方向重复和方差。正式设置为 $c=0.005$，每步 8 个 score 端点在 4 张 GPU 上并行计算。
...
经验设置为 $\eta=0.01$、$(\beta_1,\beta_2)=(0.5,0.999)$。较小 $\beta_1$ 使偶发坏方向不会长时间污染一阶动量。
...
采用与上一节完全相同的 128 起点、200 步、$\eta=0.01$、$c=0.005$、四方向和 RK4-256 设置，仅把条件改为 $N_{\rm FP}=6,n_c=2$，并启用时间尺度 guard。结果将在正式作业验收后追加。

## 7. 未来方向摘要

后续研究集中在三个互补方向。第一，现有 proxy 分类器能以 ROC-AUC 0.934 区分逆追踪 QUASR 潜变量与高斯先验，但与真 score 基本零相关；连续回归器测试 Pearson 约 0.40，却不能识别真实高分尾部。因此 proxy 只有在使用 ABI-9 多任务标签、按高分分层训练和以 top-$k$ 真 score 富集验收后才值得重做。
...
第三，评分器可返回局部近似梯度。axis 候选与最大面选择难以全局反传，且在高分区通常已饱和；coil 工程项容易求导但不是主要瓶颈；最有价值的是固定可行分支后对 volume-QS 及 $\alpha/\iota$ 线性最小二乘求 VJP，再通过可微 flow ODE 回传到潜空间。该近似梯度只负责提出方向，真实 ABI-9 score 继续负责接受和拓扑门控。详细推导、风险和分阶段验收方案见[后续研究方向可行性分析](../reports/qh_future_directions_feasibility.md)。
...
3. flow matching 的直接生成质量不足，但其可逆重参数化对潜空间优化有实用价值；SPSA 型四方向估计加低动量 Adam 已产生经完整物理支线验收的高分 QH 解。

仍需保留的限制是：快速 score 的固定 $a$ 与定长筛面只适合排序，不能代替逐样本最大可行面搜索；评分函数含拓扑和候选选择分支，不是全局光滑函数；ABI-9 修复前的 volume-QS、landscape、proxy 标签和优化分数均不可与当前分数直接比较。
